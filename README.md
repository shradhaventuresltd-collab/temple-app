...
3. Grant the claim:
   ...
   **Research photo packs (KAN-77+):** verified Commons jpgs live under `tools/photo-packs/{slug}/` with `manifest.json` + `ATTRIBUTION.md`. That includes the five packs from #19 and the 20 Karnataka Wave B packs. Upload to Storage (no Places scraper). This does not write live Firestore unless you pass `--patch-firestore`, and it needs a local `service-account.json` (gitignored). Do not invent Storage URLs.
   ...
   `--all-packs` uploads every pack in `tools/photo-packs/` and requires the 20 Karnataka Wave B folders. An empty manifest (Bengaluru Banashankari, while Research regenerates it) is skipped: no upload and no Firestore patch. Sample/Seed may already use Commons `downloaded_url` thumbs so honesty clears before Storage; after upload, replace `imageUrl` / `images` with the printed Storage URLs (or let `--patch-firestore` write live docs that already exist). Seed first in a debug admin session so those docs exist. One pack folder differs from the Seed id: `cheluvanarayana-swamy-temple-melukote` uploads to `temples/cheluvanarayana-swamy-temple/`. Full table: [`tools/photo-packs/README.md`](tools/photo-packs/README.md).
