...
3. Grant the claim:
   ...
   **Research photo packs (KAN-77+):** verified Commons files live under `tools/photo-packs/{slug}/` with `manifest.json` + `ATTRIBUTION.md`. That includes the five packs from #19, the 20 Karnataka Wave B packs, and the 35 Tamil Nadu + Kerala Wave A packs. Upload to Storage (no Places scraper). `service-account.json` stays local and gitignored. `--patch-firestore` does not create missing documents and does not write live Firestore unless you pass it. Do not invent Storage URLs.

   ```bash
   cd scripts
   npm install
   node upload_research_pack.js --all-packs --dry-run
   node upload_research_pack.js --all-packs
   node upload_research_pack.js --all-packs --patch-firestore
   ```

   `--all-packs` uploads every pack in `tools/photo-packs/` and requires the 20 Karnataka Wave B folders and the 35 Tamil Nadu + Kerala Wave A folders. An empty manifest is skipped and does not patch Firestore. Sample/Seed may already use Commons `downloaded_url` thumbs so honesty clears before Storage; after upload, replace `imageUrl` / `images` with the printed Storage URLs (or let `--patch-firestore` write live docs that already exist). Do not invent Storage URLs. Seed never writes picsum. A later debug Seed keeps a verified live gallery (admin or Storage) and applies the pack gallery only when that live document has no verified image URLs. Seed first in a debug admin session so those docs exist. `cheluvanarayana-swamy-temple-melukote` uploads to `temples/cheluvanarayana-swamy-temple/`. Wave A pack folders that differ from `templeDocumentId` (Srirangam, Palani, Thiruchendur, Thiruvanaikaval, Tiruvarur, Tirunelveli) upload under the Seed id. Thiruchendur’s pack is site-verified (eight Tiruchendur frames) and uploads; it is no longer an empty skip. See `tools/photo-packs/README.md`.
...
The original 10 Tamil Nadu, 10 Kerala, and 10 Karnataka sample temples carry research-enriched story, timings (with source caveats), specialities, address, and map pins. KAN-74 adds +10 more temples each for Tamil Nadu, Kerala, and Karnataka (batches 16–18). Expansion batches also add 10 temples each for Andhra Pradesh, Telangana, Gujarat, Odisha, Uttar Pradesh, Jammu and Kashmir, Maharashtra, Rajasthan, West Bengal, Madhya Pradesh, Bihar, and Himachal Pradesh, with the same field mapping (`location` stored as `address`) and the same honesty caveats kept in `timings` / `story`. KAN-77 replaces picsum or empty covers with Commons `downloaded_url` thumbs where a verified photo pack is on file (priority packs, Karnataka Wave B, and Tamil Nadu + Kerala Wave A). Temples without a pack still use the earlier picsum or empty cover. Gallery `images` in the sample are those Commons thumbs until a real Storage upload; Seed does not copy picsum into Firestore. **Hybrid C:** `trip_planning` is not in the Dart model or Seed. Sources and Commons photo candidates live in [`docs/tamil-nadu-sample-research.md`](docs/tamil-nadu-sample-research.md), [`docs/kerala-sample-research.md`](docs/kerala-sample-research.md), [`docs/karnataka-sample-research.md`](docs/karnataka-sample-research.md), [`docs/andhra-pradesh-sample-research.md`](docs/andhra-pradesh-sample-research.md), [`docs/telangana-sample-research.md`](docs/telangana-sample-research.md), [`docs/gujarat-sample-research.md`](docs/gujarat-sample-research.md), [`docs/odisha-sample-research.md`](docs/odisha-sample-research.md), [`docs/uttar-pradesh-sample-research.md`](docs/uttar-pradesh-sample-research.md), [`docs/jammu-kashmir-sample-research.md`](docs/jammu-kashmir-sample-research.md), [`docs/maharashtra-sample-research.md`](docs/maharashtra-sample-research.md), [`docs/rajasthan-sample-research.md`](docs/rajasthan-sample-research.md), [`docs/west-bengal-sample-research.md`](docs/west-bengal-sample-research.md), [`docs/madhya-pradesh-sample-research.md`](docs/madhya-pradesh-sample-research.md), [`docs/bihar-sample-research.md`](docs/bihar-sample-research.md), and [`docs/himachal-pradesh-sample-research.md`](docs/himachal-pradesh-sample-research.md) — they are not Firestore fields. Re-seeding from a debug admin client refreshes bundled text on the stable slug IDs; this repo does not ship a live Firebase seed from CI.
