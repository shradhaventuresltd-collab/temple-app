...
void main() {
  final karnataka =
      sampleTemples.where((t) => t.state == 'Karnataka').toList();
  ...
  test('Karnataka sample temples parse visitor fields without dropping covers', () {
    for (final temple in karnataka) {
  ...
  test('uncertain Karnataka timings keep source caveats', () {
    Temple named(String name) =>
        karnataka.firstWhere((t) => t.name == name);

    expect(
      named('Sri Krishna Matha (Udupi)').timings,
      contains('Paryaya'),
    );
    expect(
    ...
    expect(
      named('Kukke Subramanya Temple').timings,
      contains('HRCE'),
    );
    expect(
    ...
    expect(
      named('Srikanteshwara Temple, Nanjangud').timings,
      contains('HRCE'),
    );
    expect(
  ...
  test('Karnataka sample round-trips through Seed/CMS Firestore field names', () {
    for (final temple in karnataka) {
